...
assert s_char != -1 and e_char != -1, "drawCharacter not found!"

new_draw_character = """function drawCharacter(c, info) {
    const x = 150;
    const y = 370;
    ...
    if (isFemale) {
      ...
      // 7. Expressive Eyes, Eyelashes, Cheek Blush & Tinted Lips
      ...
      // 8. Seamless Front Hair Locks (Flowing smoothly from crown parting over collar!)
      ...
      if (archId === 'infiltrator') {
        // MAID MARIAN: Ruby Brow Tiara & Hand-Held Damascus Stiletto
        ...
        ctx.fillStyle = '#ffffff'; ctx.fillRect(0.2, -74.5, 0.7, 0.7);
        ...
        ctx.fillStyle = 'rgba(52, 211, 153, 0.7)'; ctx.font = '11px sans-serif'; ctx.fillText('✨', 14, -56);
      ...
      // 1. Wool Hosen & Sculpted Riding Boots
      ...
      // Leather Boots with Turned Cuffs
      ...
      // 2. Layered Tunic / Monk Habit / Gambeson
      ...
      // Belt: Knotted Rope Cincture for Friar, Leather Belt for others
      if (isMonk) {
      ...
      // 4. Contoured Head Geometry
      ctx.fillStyle = skinBase;
      ctx.beginPath();
      if (archId === 'brawler') {
        // Little John: Broad warrior jaw
      ...
      if (archId === 'friar') {
        ...
        // Much: Bright alert youthful eyes & freckles
      ...
      // =====================================
      // --- CHARACTER-SPECIFIC ART & HAIR ---
      // =====================================

      if (archId === 'friar') {
        // --- FRIAR TUCK (MASTERPIECE MONK ARTISAN OVERHAUL) ---
        if (c.isBeggar) {
          // Beggar Friar Disguise
          ctx.fillStyle = '#3e2717';
          ctx.beginPath(); ctx.ellipse(0, -74, 11, 7.5, 0, Math.PI, 0); ctx.fill();
          ctx.fillStyle = '#5c3a21'; ctx.fillRect(-4, -78, 4, 3);
          ...
          ctx.fillStyle = skinBase;
          ctx.beginPath();
          ctx.ellipse(0, -73.5, 6.2, 3.8, 0, 0, Math.PI * 2);
          ctx.fill();
          // Subtle healthy scalp shine
          ctx.fillStyle = 'rgba(255, 255, 255, 0.35)';
          ctx.beginPath(); ctx.ellipse(1.5, -74.5, 3, 1.5, 0.2, 0, Math.PI * 2); ctx.fill();

          // 2. Sculpted Tonsure Ring of Curly Hair (Hugs the skull from ear to ear!)
          ctx.fillStyle = hairSh;
          // Left Hair Temple & Ear Fluff
          ctx.beginPath();
          ctx.moveTo(-5.5, -74);
          ctx.quadraticCurveTo(-9.2, -73, -9.6, -67);
          ctx.quadraticCurveTo(-10.2, -62, -7.5, -60);
          ctx.lineTo(-5.5, -62);
          ctx.quadraticCurveTo(-8, -66, -5.5, -74);
          ctx.closePath(); ctx.fill();

          // Right Hair Temple & Ear Fluff
          ctx.beginPath();
          ctx.moveTo(5.5, -74);
          ctx.quadraticCurveTo(9.2, -73, 9.6, -67);
          ctx.quadraticCurveTo(10.2, -62, 7.5, -60);
          ...
          // Back Tonsure Ring connecting behind the bald crown
          ctx.fillStyle = hairHex;
          ctx.beginPath();
          ctx.arc(0, -73, 8.8, -Math.PI * 0.92, -Math.PI * 0.08);
          ctx.quadraticCurveTo(0, -71, -8.8 * Math.cos(-Math.PI * 0.08), -73 + 8.8 * Math.sin(-Math.PI * 0.08));
          ctx.stroke();

          // Curled lock strands with volume highlights
          ctx.strokeStyle = hairHi; ctx.lineWidth = 1.3;
          ctx.beginPath();
          ctx.arc(-7.8, -66, 2.5, -Math.PI * 0.5, Math.PI * 0.5);
          ctx.arc(7.8, -66, 2.5, Math.PI * 0.5, -Math.PI * 0.5);
          ctx.stroke();

          // 3. Thick Monastic Cowl draped around neck
          ...
          // 4. Carved Oak Ale Tankard firmly gripped in hand
        ...
        // --- ROBIN HOOD: Lincoln Green Bycocket, Feathers & Hand-Held Longbow ---
        if (c.isBeggar) {
          ctx.fillStyle = '#5c4d3c';
          ctx.beginPath(); ctx.ellipse(0, -75, 11, 7, 0, Math.PI, 0); ctx.fill();
          ctx.fillStyle = '#7a6852'; ctx.fillRect(-4, -78, 4, 3);
          ctx.fillStyle = '#6e4726'; ctx.beginPath(); ctx.roundRect(4, -46, 9, 8, 2); ctx.fill();
          ctx.fillStyle = '#cbd5e1'; ctx.beginPath(); ctx.arc(7, -44, 1.5, 0, Math.PI * 2); ctx.fill();
          ctx.fillStyle = skinBase; ctx.beginPath(); ctx.arc(3, -42, 2.5, 0, Math.PI * 2); ctx.arc(14, -42, 2.5, 0, Math.PI * 2); ctx.fill();
        } else {
          ctx.fillStyle = hatHex;
          ctx.beginPath();
          ...
          // FIRM HAND-HELD LONGBOW (NO FLOATING BOWS!)
          ...
          ctx.fillStyle = skinBase;
          ctx.beginPath(); ctx.roundRect(bowHandX - 2.5, bowHandY - 3, 5, 6, 1.5); ctx.fill();
        }
        ...
        // --- WILL SCARLET: Cavalier Feathered Hat & Rapier Basket Hilt ---
        ...
        // --- GUY OF GISBORNE: Nasal Helmet & Blackened Broadsword ---
        ...
        // --- SHERIFF OF NOTTINGHAM: Velvet Cap, Ermine Fur & Tax Purse ---
...
text = text[:s_char] + new_draw_character + text[e_char:]
print("Sculpted all artisan characters in simulator.html!")
