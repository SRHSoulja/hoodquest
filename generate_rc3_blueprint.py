...
def build_section_0():
    return r"""# HoodQuest (HQ): Production System Specification (Protocol Frozen V1.0-RC3)
...
- **No Semantic Mutation Covenant:** Numeric constants, item IDs, item names, supply modes, recipes, drop rates, stat effects, companion rewards, starvation percentages, event dates, and previously approved gameplay formulas are immutable.
...
- **`HoodQuest.sol` (ERC-721 + `ERC721Enumerable` + EIP-4906 Core + Receivers):**
  ...
  - **Canonical Core Gating Counters & Pre-Sale Gate (`canList`) (HQ-CORE-004, HQ-CORE-005):**
    ...
    function incrementSolsticeCommitment(uint256 hoodId) external onlyAuthorizedEngine {
...
function _resolveSequentialRoll(
    ...
    // 4. Fixed-Point Gold Carry Accumulator (HQ-ECO-001)
...
function _accrueRewardBundle(uint256 hoodId, RewardBundle memory bundle) internal {
    ...
    bool finalBundleNonZero = (
        ...
        bundle.baskets > 0 || bundle.elixirs > 0 ||
...
### 2. Event Kinds & Zero-Keeper World Event Synchronization (`syncWorldEvents`):
```solidity
...
### 3. Unified Reusable `ActionRequest` Engine & 5 Tactical Sectors (HQ-EVT-003):
```solidity
...
### 4. Executable Solstice Championship (Event-Scoped & Expirable - HQ-EVT-004):
```solidity
...
| **HQ-CORE-009** | Biological Clock | Effective age advances only when awake; derived slumber pauses age with zero gas. | `[Present - Section 7]` |
...
| **HQ-ECO-001**   | Fixed-Point Gold | Starvation and Gold-bonus carry evaluated with fixed-point `goldCarry` accumulator. | `[Present - Section 3]` |
